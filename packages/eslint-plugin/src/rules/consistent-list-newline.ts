...
export default createEslintRule<Options, MessageIds>({
  ...
  create: (context, [options = {}] = [{}]) => {
    ...
    function getDelimiter(
      ...
      if (
        root.type !== 'TSInterfaceDeclaration' &&
        root.type !== 'TSTypeLiteral'
      )
        return;
      const currentContent = context.sourceCode.text.slice(
        current.range[0],
        current.range[1]
      );
    ...
    function hasComments(current: TSESTree.Node): boolean {
      ...
      while (program.type !== 'Program') program = program.parent;
      const currentRange = current.range;

      return !!program.comments?.some((comment) => {
        const commentRange = comment.range;
        return (
          commentRange[0] > currentRange[0] && commentRange[1] < currentRange[1]
        );
      });
    }
      ...
      JSXOpeningElement(node) {
        if (
          node.attributes.some(
            (attr) => attr.loc.start.line !== attr.loc.end.line
          )
        )
          return;
