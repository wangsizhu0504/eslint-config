...
export default createEslintRule<Options, MessageIds>({
  ...
  create: (context) => {
    ...
    const { allowLeadingPropertyAccess = true } = context.options[0] || {};
    ...
    return {
      MemberExpression(node) {
        ...
        while (
          root.parent &&
          (root.parent.type === 'MemberExpression' ||
        ...
        members.forEach((m) => {
          ...
          const currentMode: 'single' | 'multi' =
            token.loc.start.line === tokenBefore.loc.end.line
              ? 'single'
              : 'multi';

          if (
            leadingPropertyAcccess &&
            (m.object.type === 'ThisExpression' ||
              m.object.type === 'Identifier' ||
              m.object.type === 'MemberExpression' ||
              m.object.type === 'Literal') &&
            currentMode === 'single'
          ) {
            return;
          }
